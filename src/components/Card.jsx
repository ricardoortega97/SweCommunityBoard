import react from "react";
import Event from "./Event";

const Card = () => {
    return (
        <div className="Container revealable">
            
            {/* CS50 */}
            <Event 
                name="CS50 by Harvard" 
                description="Dive deep with Harvard's Introduction to CS50 Programs that 
                offer a verity options to choose such as Python, Web Development, AI, and more!" 
                imgSrc="https://pll.harvard.edu/themes/custom/twel_scholar/logo.svg" 
                link="https://www.edx.org/cs50" 
                bgColor="red"
            />
             {/* Swift */}
             <Event
                name="Hacking with Swift"
                description="Learn to buiild iOS apps using SwiftUI. This is a course is broken down into 100 days with guides, videos,
                challenges and more."
                imgSrc="https://www.hackingwithswift.com/img/logo-curves-ua.svg"
                link="https://www.hackingwithswift.com/learn"
                bgColor="red"
            />
            {/* Mmdn_ */}
            <Event
                name="Mmdn_"
                description="This site is a great resource guid for Web Development, provides examples the structures for web content with demos
                to play around."
                imgSrc="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRWQ-8NEdJGOdPtwBU5F5bb5d93PZAuvjnbyg&s"
                link="https://developer.mozilla.org/en-US/"
                bgColor="dark-blue"
            />
             {/* W3 Schools */}
             <Event
                name="W3 Schools"
                description="This site also focuses on Web Development with a vertiy of resources,
                 but as well other programming languages like Python, Java, and SQL"
                imgSrc="data:image/png;base64,"
                link="https://www.w3schools.com/"
                bgColor="light-green"
            />
            {/* CodePath */}
            <Event
                name="CodePath"
                description="Helping underrepresented CS students seeking tech careers providing guidence, training, mentorship, and
                being a part of the community."
                imgSrc="https://www.codepath.org/hs-fs/hubfs/logo-codepath.png?width=538&height=48&name=logo-codepath.png"
                link="https://www.codepath.org/"
                bgColor="green"
            />
            {/* Codedex */}
            <Event
                name="Codedex"
                description="A fantasy land that you journey your way with learning to code with Python, JavaScript and gain XP
                to unlock new regions at your own pace."
                imgSrc="https://www.codedex.io/images/coin-cropped.png"
                link="https://www.codedex.io/"
                bgColor="dark-blue"
            />
            {/* GitHub Learning */}
            <Event
                name="GitHub Learning Lab"
                description="Improve your GitHub skills with the Github learning lab with the guidence of a bot that will 
                assist with feedback on your repositories work."
                imgSrc="data:image/png;base64,"
                link="https://github.com/apps/github-learning-lab"
                bgColor="Whitesmoke"
            />
             {/* NeedCode */}
             <Event
                name="NeetCode"
                description="Get familiar with coding interviews and prepare with NeetCode. This site has a roadmap if you are not 
                sure where to begin. "
                imgSrc="https://neetcode.io/assets/neetcode-io-logo.png"
                link="https://neetcode.io/"
                bgColor="grey"
            />
            {/* CodeWars */}
           <Event 
                name="Codewars"
                description="Improve your development skills with your peers or compete with them as well with challenging problems.
                 Push yourself to the limits and build self-confidence."
                imgSrc="https://cdn.prod.website-files.com/6674f0cdb5b7b401612cf015/6674f0cdb5b7b401612cf0ee_codewars-by-andela-dark.svg"
                link="https://www.codewars.com/"
                bgColor="dark-red"
           />
             {/* LeetCode */}
             <Event 
                name="LeetCode" 
                description="Once you have learned a programming language of your chose, prepare for technical interview
                 questions on LeetCode and enhance skils and knowledge." 
                imgSrc="https://leetcode.com/static/images/LeetCode_logo_rvs.png" 
                link="https://leetcode.com/" 
                bgColor="black"
            />
        </div>
    )
}

export default Card;